...
All the groups have different means and groups 1 and 3 seem to contain one outlier value. Now we must conduct analysis of variance (ANOVA) to study whether the differences between groups are statistically significant. Here the null hypothesis is that the group means do not differ. 
...
According to the ANOVA test the p-value is close to zero so we can infer that the groups differ in group means.
